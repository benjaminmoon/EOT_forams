---
title: "Data and code from 'Surviving cliate change: morphological trends in reticulate _Nummulites_ cross the Eocene-Oligocene transition'"
...
KIRAN KOORAPATI, R., MOON, B. C. and COTTON, L. 2024. Data from 'Surviving cliate change: morphological trends in reticulate _Nummulites_ cross the Eocene-Oligocene transition'. Zenodo1, doi: 10.5281/zenodo.13623171
...
main_timeplot_isotopes <- function(prefix, time_data = timeser_dat) {
  time_data |>
    ...
    annotate(
      "segment",
      x = 33.9, xend = 33.5, y = -Inf, yend = -Inf,
      colour = "#444444", lwd = 3, inherit.aes = FALSE
    ) +
    annotate(
      "text",
      x = 33.5, y = -Inf, label = "EOT",
      vjust = -0.2, hjust = -0.1, size = 2.5
    ) +
    geom_line() +
    scale_x_reverse(limits = rev(c(33.1, 34.1))) +
    xlab("Age (Ma)") +
    ylab(prefix[[2]])
}
